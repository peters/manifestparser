...
var Apk2XML = {
	...
	endTag : 0x00100103,
	/**
	 * LEW Return value of a Little Endian 32 bit word from the byte array at
	 * offset off
	 * 
	 * @param {Buffer}
	 *            buffer
	...
	completeXMLstringAt : function completeXMLStringAt(buffer, strOff) {
		var strLength = buffer.readUInt32LE(strOff + 1) << 8 & 0xff00
				| buffer.readUInt32LE(strOff) & 0xff, chars = new Buffer(
				strLength, 'utf-8');
	...
	parse : function(apkReader, buffer) {
		...
		// Local variables
		var off = XMLTagOff, startTagLineNo = -2, tag0, lineNo, nameNsSi, nameSi,
			name, tag6, numbAttrs, stringBuilder, attrNameNsSi, attrNameSi, attrValueSi, 
			strOff, attrName, attrValue, attrResId;
		...
		while (off < buffer.length - 25) {
			...
			if (tag0 === Apk2XML.startTag) {
				...
				for ( var i = 0; i < numbAttrs; i++) {
					...
					attrValue = attrValueSi !== -1 ? Apk2XML.completeXMLString(
							buffer, sitOff, stOff, attrValueSi)
							: "resourceID 0x"
									+ parseInt(attrResId, 16).toString();
