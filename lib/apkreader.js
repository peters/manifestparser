...
/**
 * Extract android manifest from apk files. Original implementation in Java
 * can be found here: http://pastebin.com/c53DuqMt
 */
var Apk2XML = {
	/**
	...
	lew: function lew(buffer, off) {
		return buffer.readUInt32LE(off + 3) << 24 & 0xff000000 | buffer.readUInt32LE(off + 2) << 16 & 0xff0000
	                | buffer.readUInt32LE(off + 1) << 8 & 0xff00 | buffer.readUInt32LE(off) & 0xFF;
	},
	/**
	 ...
	 * .
	 * @param {Buffer} buffer
	 * @param {Integer} strOff
	 */
	completeXMLstringAt: function completeXMLStringAt(buffer, strOff) {
		var strLength = buffer.readUInt32LE(strOff + 1) << 8 & 0xff00 | buffer.readUInt32LE(strOff) & 0xff,
	 ...
	 * 
	 * @param {Array} buffer
	 * @param {Integer} sitOff
	 * @param {Integer} stOff
	 * @param {Integer} strInd
	 */
	...
	parse: function(apkReader, buffer) {
		...
		while(off < buffer.length - 25) {
			...
			if(tag0 === Apk2XML.startTag) {
				...
				for(var i = 0; i < numbAttrs; i++) {
					...
					// Name of attribute
					attrName = Apk2XML.completeXMLString(buffer, sitOff, stOff, attrNameSi);
					
					// Value of XML attribute (attrName)
					attrValue = attrValueSi !== -1 ? 
						Apk2XML.completeXMLString(buffer, sitOff, stOff, attrValueSi) : 
		...
		var XMLString = finalXML.join(''),
			stdout = apkReader.options.outputTarget === 'stdout';
		...
		switch(apkReader.options.outputFormat) {
			...
			case 'xml':
				if(stdout) {
					process.stdout.write(prettyXML(XMLString));
					return;
				} 
				apkReader.emit('manifest', prettyXML(XMLString));
			break;
			default:
				apkReader.emit('error', new Error("Unsupported output format " + apkReader.options.outputFormat));
				return;
			break;
		}		
	}
...
ApkManifestReader.prototype.parse = function() {
	...
	try {
		...
		self.emit('error', new Error("Zip library was unable to read apk file " + this.target));
		return;
	}
	...
	if(manifest === null) {
		this.emit('error', new Error("AndroidManifest.XML not found in target " + this.target));
		return this;
	}
